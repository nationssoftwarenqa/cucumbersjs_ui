Feature: Bible Web App Ask A Question

        @tstsev
        @tst
        @stgsev
        @stg
        Scenario: Submit A Question
            Given I visit the online bible app
             Then click on the "humbergarMenu" element
             Then I wait for "1" seconds
             Then click on the "SignIn" element
             Then click on the "questionlink" element
             Then I wait for "3" seconds
             When I type "Nations" in the field "questionsfirstname"
             When I type "Quality" in the field "questionslastname"
             When I type "nationsquality@gmail.com" in the field "questionsemail"
             Then select "English" from the "questionslanguage" drowpdown
             Then select "Web - Bible.com" from the "questionsplatform" drowpdown
             When I type "Chrome" in the field "questionsBrowerType"
             Then select "Other Question" from the "questionsTypeOfQuestion" drowpdown
             When I type "The Bible App Is Wonderful" in the field "questionsTextArea"
             #Then click on the "Submit" element

        @tstsev
        @tst
        @stgsev
        @stg
        Scenario: Submit A Question From Forgot Password Page
            Given I visit the online bible app
             Then click on the "humbergarMenu" element
             Then I wait for "1" seconds
             Then click on the "SignIn" element
             Then click on the "questionlink" element
             Then I wait for "3" seconds
             When I type "Nations" in the field "questionsfirstname"
             When I type "Quality" in the field "questionslastname"
             When I type "nationsquality@gmail.com" in the field "questionsemail"
             Then select "English" from the "questionslanguage" drowpdown
             Then select "Web - Bible.com" from the "questionsplatform" drowpdown
             When I type "Chrome" in the field "questionsBrowerType"
             Then select "Other Question" from the "questionsTypeOfQuestion" drowpdown
             When I type "The Bible App Is Wonderful" in the field "questionsTextArea"
             #Then click on the "Submit" element
            